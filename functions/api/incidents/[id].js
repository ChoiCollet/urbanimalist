...
export async function onRequestPut({ request, env, params }) {
  ...
  const before = await env.DB.prepare(`SELECT * FROM incidents WHERE id = ?`).bind(params.id).first();
  if (!before) {
  ...
  await env.DB.prepare(
    ...
    .bind(after.sido, after.sigungu, after.cause, after.title, after.desc, after.source, after.source_url, after.date, params.id)
    .run();

  await logAction(env, { map: "map1", entityId: params.id, action: "update", before, after });
...
export async function onRequestDelete({ request, env, params }) {
  ...
  await logAction(env, { map: "map1", entityId: params.id, action: "delete", before, after: null });
